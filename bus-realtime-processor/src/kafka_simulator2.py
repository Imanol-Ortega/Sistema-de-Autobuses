...
# ------------------- SIMULADOR -------------------
class BusSimulator:
    # (todo tu código anterior del simulador va aquí sin cambios, pero quitamos el print del método `update`)
    ...
    def update(self):
        ...
        for bus in self.buses:
            if bus["status"] == "in_operation":
                self._move_bus(bus)
            # Eliminamos los prints aquí
            self._check_if_at_stop(bus)

    def _check_if_at_stop(self, bus):
        position = tuple(bus["current_position"])
        for lat, lon, name in self.landmarks:
            if geodesic(position, (lat, lon)).meters < 20:
                bus["at_stop"] = name
                return
        bus["at_stop"] = None
    ...
    def _move_bus(self, bus):
        ...
        t = 1 / 3600  # tiempo en horas (1 segundo)
        d = effective_speed * t  # distancia a recorrer en 1 segundo
...
def send_bus_data_to_kafka(producer, buses):
    for bus in buses:
        msg = {
            "bus_id": bus["id"],
            "plate": bus["plate"],
        ...
        try:
            ...
            print(f"[Bus {bus['id']}] ❌ ERROR al enviar a Kafka:", e)

# ------------------- EJECUCIÓN -------------------
def main():
    simulator = BusSimulator(num_buses=6)
    producer = create_kafka_producer()

    print("📡 Simulador de buses ejecutándose y enviando datos a Kafka...")

    try:
        while True:
            simulator.update()
            buses_data = simulator.get_bus_data()
            send_bus_data_to_kafka(producer, buses_data)
